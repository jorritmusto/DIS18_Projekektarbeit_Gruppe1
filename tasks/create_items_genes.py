...
"""
This script defines a Luigi task that processes gene data from an Excel file, formats it for quick statements, and prepares it for uploading to a Wikibase instance.
"""
...
class CreateItemsGenesTask(luigi.Task):
    ...
    def run(self):
        ...
        df_genes = (
            df_genes.groupby(["Locus_tag", "Product", "GeneLength"])
            .head(1)
        ...
        df_genes = df_genes.rename(
            columns={"Locus_tag": "Len", "Product": "Den", "GeneLength": "P7"}
        )
        ...
        df_genes.to_csv(
            "data/quick_statements/genes/qs_genes.csv", sep=",", index=False
        )
