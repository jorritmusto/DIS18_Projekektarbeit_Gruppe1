...
"""
This script defines a Luigi task that creates initial items for genes, transcription start sites (TSS), and conditions in a Wikibase instance, checking for existing items and logging the results.
"""
...
class CreateInitialItemsTask(luigi.Task):
    ...
    def run(self):
        ...
        if not df["label"].str.contains(len_tss).any():
            ...
            df_tss_item = self.create_statments_df(
                Len=len_tss, Aen=aen_tss, Den=den_tss
            )
        ...
        # if item already exists -> write a message that the item already exists + qid
            ...
            with open(
                "data/quick_statements/initial_items/tss.txt", "w"
            ) as output_file:
                output_file.write(
                    ("Item {} already exists with qid: {}").format(len_tss, qid)
                )
        ...
        if not df["label"].str.contains(len_gene).any():
            ...
            df_gene_item = self.create_statments_df(
                Len=len_gene, Aen=aen_gene, Den=den_gene
            )
        ...
        # if item already exists -> write a message that the item already exists + qid
            ...
            with open(
                "data/quick_statements/initial_items/gene.txt", "w"
            ) as output_file:
                output_file.write(
                    ("Item {} already exists with qid: {}").format(len_gene, qid)
                )
        ...
        if not df["label"].str.contains(len_M63_04).any():
            ...
            df_M63_04 = self.create_statments_df(
                Len=len_M63_04, Aen=aen_M63_04, Den=den_M63_04
            )
            print(df_M63_04)
            # drop alias column
        ...
        # if item already exists -> write a message that the item already exists + qid
            ...
            with open(
                "data/quick_statements/initial_items/M63_04.txt", "w"
            ) as output_file:
                output_file.write(
                    ("Item {} already exists with qid: {}").format(len_M63_04, qid)
                )
        ...
        if not df["label"].str.contains(len_LB_04).any():
            ...
            df_LB_04 = self.create_statments_df(
                Len=len_LB_04, Aen=aen_LB_04, Den=den_LB_04
            )
        ...
        # if item already exists -> write a message that the item already exists + qid
            ...
            with open(
                "data/quick_statements/initial_items/LB_04.txt", "w"
            ) as output_file:
                output_file.write(
                    ("Item {} already exists with qid: {}").format(len_LB_04, qid)
                )
        ...
        if not df["label"].str.contains(len_LB_20).any():
            ...
            df_LB_20 = self.create_statments_df(
                Len=len_LB_20, Aen=aen_LB_20, Den=den_LB_20
            )
        ...
        # if item already exists -> write a message that the item already exists + qid
            ...
            with open(
                "data/quick_statements/initial_items/LB_20.txt", "w"
            ) as output_file:
                output_file.write(
                    ("Item {} already exists with qid: {}").format(len_LB_20, qid)
                )
    ...
    def create_statments_df(self, Len, Aen, Den):
        """
            ...
            items 
        """
        df = pd.DataFrame({"qid": [""], "Len": [Len], "Aen": [Aen], "Den": [Den]})
        return df
